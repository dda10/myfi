...
func TestRecordTransaction_AllTypes(t *testing.T) {
	...
	types := []TransactionType{Buy, Sell, Deposit, Withdrawal, Interest, Dividend}
...
func TestRecordTransaction_ValidationErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "zero user ID",
			tx: Transaction{
				UserID: 0, AssetType: VNStock, Symbol: "FPT",
				Quantity: 10, UnitPrice: 85000, TransactionDate: time.Now(), TransactionType: Buy,
			},
		...
		},
		{
			name: "empty symbol",
			tx: Transaction{
				UserID: 1, AssetType: VNStock, Symbol: "",
				Quantity: 10, UnitPrice: 85000, TransactionDate: time.Now(), TransactionType: Buy,
			},
		...
		},
		{
			...
			tx: Transaction{
				...
				Quantity: 0, UnitPrice: 85000, TransactionDate: time.Now(), TransactionType: Buy,
			},
		...
		},
		{
			...
			tx: Transaction{
				UserID: 1, AssetType: VNStock, Symbol: "FPT",
				Quantity: 10, UnitPrice: -100, TransactionDate: time.Now(), TransactionType: Buy,
			},
		...
		},
		{
			...
			tx: Transaction{
				UserID: 1, AssetType: AssetType("forex"), Symbol: "USDVND",
				Quantity: 10, UnitPrice: 25000, TransactionDate: time.Now(), TransactionType: Buy,
			},
...
func TestGetTransactionsBySymbol(t *testing.T) {
	...
	for _, sym := range []string{"FPT", "FPT", "SSI"} {
		ledger.RecordTransaction(ctx, Transaction{
			UserID: 1, AssetType: VNStock, Symbol: sym,
			Quantity: 10, UnitPrice: 85000, TransactionDate: time.Now(), TransactionType: Buy,
		})
	}
