...
func TestPriceService_GetQuotes_VNStock(t *testing.T) {
	...
	for _, quote := range quotes {
		...
		if quote.AssetType != VNStock {
			t.Errorf("Expected asset type %s, got %s", VNStock, quote.AssetType)
		}
		if quote.Source == "" {
...
func TestPriceService_GetQuotes_Cache(t *testing.T) {
	...
	start := time.Now()
	quotes1, err := priceService.GetQuotes(ctx, symbols, VNStock)
	if err != nil {
	...
	start = time.Now()
	quotes2, err := priceService.GetQuotes(ctx, symbols, VNStock)
	if err != nil {
...
func TestPriceService_UnsupportedAssetType(t *testing.T) {
	...
	_, err = priceService.GetQuotes(ctx, symbols, AssetType("invalid"))
	if err == nil {
