...
func (k *KnowledgeBase) StoreObservation(ctx context.Context, obs *model.PatternObservation) error {
	if k.db == nil {
...
func (k *KnowledgeBase) getObservationsNeedingOutcomeUpdate(ctx context.Context) ([]model.PatternObservation, error) {
	now := time.Now()

	// Find observations where:
	// - outcome_1day is NULL and detection_date is at least 1 day ago
	// - outcome_7day is NULL and detection_date is at least 7 days ago
	// - outcome_14day is NULL and detection_date is at least 14 days ago
	// - outcome_30day is NULL and detection_date is at least 30 days ago
	query := `
	...
	rows, err := k.db.QueryContext(ctx, query,
		...
		now.AddDate(0, 0, -14), // 14 days ago
...
func (k *KnowledgeBase) updateObservationOutcome(ctx context.Context, obs *model.PatternObservation) error {
	now := time.Now()
	...
	// Update 1-day outcome if applicable
	...
	// Update 7-day outcome if applicable
	...
	// Update 14-day outcome if applicable
	...
	// Update 30-day outcome if applicable
...
// Uses ObservationFilters defined in monitor_agent.go.
func (k *KnowledgeBase) QueryObservations(ctx context.Context, filters ObservationFilters) ([]model.PatternObservation, error) {
	if k.db == nil {
...
type AccuracyMetrics struct {
	...
	SuccessRate7Day   float64           `json:"successRate7Day"`  // Primary success metric
	SuccessRate30Day  float64           `json:"successRate30Day"` // Long-term success metric
}

// GetAccuracyMetrics computes aggregate accuracy metrics for a pattern type.
func (k *KnowledgeBase) GetAccuracyMetrics(ctx context.Context, patternType model.PatternType) (*AccuracyMetrics, error) {
	if k.db == nil {
		return nil, fmt.Errorf("database not configured")
	}
...
func (k *KnowledgeBase) GetAllAccuracyMetrics(ctx context.Context) ([]AccuracyMetrics, error) {
	patternTypes := []model.PatternType{
...
// Success is defined as price moving in the predicted direction (positive for accumulation/breakout).
func (k *KnowledgeBase) GetHistoricalSuccessRate(ctx context.Context, patternType model.PatternType) (float64, int, error) {
...
// This maintains the 2-year retention policy while keeping the database manageable.
...
// Used by Supervisor_Agent to incorporate knowledge base insights.
func (k *KnowledgeBase) GetRecentObservationsForSymbol(ctx context.Context, symbol string, days int) ([]model.PatternObservation, error) {
...
func (k *KnowledgeBase) GetObservationByID(ctx context.Context, id int64) (*model.PatternObservation, error) {
	if k.db == nil {
		return nil, fmt.Errorf("database not configured")
	}
