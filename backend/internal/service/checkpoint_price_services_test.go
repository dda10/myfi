...
func TestCheckpoint_VNStock_RealSymbols(t *testing.T) {
	...
	for _, q := range quotes {
		...
		if q.AssetType != model.VNStock {
			t.Errorf("Expected asset type %s, got %s", model.VNStock, q.AssetType)
		}
		if q.Source == "" {
...
func TestCheckpoint_GoldPrice_ViaPrice_Service(t *testing.T) {
	...
	quotes, err := priceService.GetQuotes(ctx, []string{"SJC", "9999"}, model.Gold)
	if err != nil {
	...
	for _, q := range quotes {
		...
		if q.AssetType != model.Gold {
			t.Errorf("Expected asset type %s, got %s", model.Gold, q.AssetType)
		}
		if q.Price <= 0 {
...
func TestCheckpoint_CacheTTL_GoldPrices(t *testing.T) {
	...
	goldPrices := []model.GoldPriceResponse{
...
func TestCheckpoint_CacheTTL_CryptoPrices(t *testing.T) {
	...
	cryptoPrices := []model.CryptoPriceResponse{
...
func TestCheckpoint_FullPipeline_AllAssetTypes(t *testing.T) {
	...
	priceService := NewPriceService(router, cache, goldService)
	cryptoService := NewCryptoService(cache, rateLimiter, infra.NewCircuitBreaker(3, 60*time.Second))

	ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
	defer cancel()

	// 1. VN Stocks
	stockQuotes, err := priceService.GetQuotes(ctx, []string{"FPT"}, model.VNStock)
	if err != nil {
	...
	// 2. Gold
	goldQuotes, err := priceService.GetQuotes(ctx, []string{"SJC"}, model.Gold)
	if err != nil {
