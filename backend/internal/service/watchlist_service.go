...
func (s *WatchlistService) CreateWatchlist(userID int, name string) (*model.Watchlist, error) {
	if name == "" {
	...
	w.Symbols = []model.WatchlistSymbol{}
	return &w, nil
}

// RenameWatchlist renames an existing watchlist owned by the given user.
func (s *WatchlistService) RenameWatchlist(userID, watchlistID int, newName string) error {
	if newName == "" {
...
func (s *WatchlistService) DeleteWatchlist(userID, watchlistID int) error {
	result, err := s.db.Exec(
...
func (s *WatchlistService) GetWatchlists(userID int) ([]model.Watchlist, error) {
	rows, err := s.db.Query(
...
func (s *WatchlistService) GetWatchlistSymbols(watchlistID int) ([]model.WatchlistSymbol, error) {
	rows, err := s.db.Query(
	...
	symbols := []model.WatchlistSymbol{}
	for rows.Next() {
		var ws model.WatchlistSymbol
...
func (s *WatchlistService) EnsureDefaultWatchlist(userID int) (*model.Watchlist, error) {
	return s.GetDefaultWatchlist(userID)
}

// GetDefaultWatchlist returns the user's default watchlist ("My Watchlist"),
// creating it with popular VN stocks if it doesn't exist yet.
func (s *WatchlistService) GetDefaultWatchlist(userID int) (*model.Watchlist, error) {
	var w model.Watchlist
	err := s.db.QueryRow(
...
func (s *WatchlistService) GetAllWatchedSymbols(userID int) ([]string, error) {
	rows, err := s.db.Query(
...
func (s *WatchlistService) SyncToMonitorAgent(userID int) error {
	symbols, err := s.GetAllWatchedSymbols(userID)
	if err != nil {
		return fmt.Errorf("failed to get watched symbols for sync: %w", err)
	}
	log.Printf("SyncToMonitorAgent: user %d has %d symbols to scan: %v", userID, len(symbols), symbols)
...
func (s *WatchlistService) createDefaultWatchlist(userID int) (*model.Watchlist, error) {
