...
type RecommendationTracker struct {
	mu            sync.RWMutex
	records       []model.RecommendationRecord // In-memory cache for recent records
	nextID        int64
...
func NewRecommendationTracker(priceService *PriceService) *RecommendationTracker {
	return &RecommendationTracker{
		records:      make([]model.RecommendationRecord, 0),
		nextID:       1,
		priceService: priceService,
		cacheMaxSize: 1000, // Keep last 1000 records in memory
	}
}

// NewRecommendationTrackerWithDB creates a tracker with database persistence.
func NewRecommendationTrackerWithDB(priceService *PriceService, db *sql.DB, kb *KnowledgeBase) *RecommendationTracker {
...
func (t *RecommendationTracker) SetKnowledgeBase(kb *KnowledgeBase) {
	t.mu.Lock()
	defer t.mu.Unlock()
	t.knowledgeBase = kb
}
...
type RecommendationAuditRecord struct {
	...
	SubAgentInputs       string    `json:"subAgentInputs"`       // JSON blob
	RecommendationOutput string    `json:"recommendationOutput"` // JSON blob
	SymbolsInvolved      string    `json:"symbolsInvolved"`      // Comma-separated
	RecommendedActions   string    `json:"recommendedActions"`   // Comma-separated
...
func (t *RecommendationTracker) LogRecommendation(ctx context.Context, rec model.AssetRecommendation, confidence int) (int64, error) {
	// Fetch current price for the symbol
	var priceAtSignal float64
	if t.priceService != nil {
		quotes, err := t.priceService.GetQuotes(ctx, []string{rec.Symbol}, model.VNStock)
	...
	record := model.RecommendationRecord{
		ID:              t.nextID,
		Symbol:          rec.Symbol,
		Action:          model.RecommendationAction(rec.Action),
...
func (t *RecommendationTracker) persistRecommendation(ctx context.Context, rec *model.RecommendationRecord) (int64, error) {
	// Build sub-agent inputs JSON
	...
	// Build recommendation output JSON
	...
	err := t.db.QueryRowContext(ctx, query,
		...
		rec.CreatedAt,
		rec.Reasoning, // Use reasoning as user query placeholder
...
// Also feeds outcomes back to Knowledge_Base (Requirement 35.5).
func (t *RecommendationTracker) UpdateOutcomes(ctx context.Context) error {
	...
	for i := range t.records {
		...
		if updated {
			...
			// Feed outcome to Knowledge_Base (Requirement 35.5)
	...
	// Also update any records in DB that aren't in memory
	if t.db != nil {
...
func (t *RecommendationTracker) updateOutcomeInDB(ctx context.Context, rec *model.RecommendationRecord) error {
	// Build outcome JSON strings
...
func (t *RecommendationTracker) updateOutcomesFromDB(ctx context.Context) error {
	...
	// Find records needing outcome updates
	query := `
	...
	for rows.Next() {
		...
		// Parse price at signal from sub_agent_inputs
		...
		// Update appropriate outcome fields
...
func (t *RecommendationTracker) feedOutcomeToKnowledgeBase(ctx context.Context, rec *model.RecommendationRecord) {
	if t.knowledgeBase == nil {
		return
	}

	// Only feed outcomes when we have meaningful data (7-day or longer)
	if rec.Return7Day == nil {
		return
	}

	// Create an observation to store the recommendation outcome
	// This helps the Knowledge_Base learn from recommendation accuracy
	obs := &model.PatternObservation{
		Symbol:           rec.Symbol,
		PatternType:      model.PatternType("recommendation_" + string(rec.Action)),
	...
	// Set outcomes
	if rec.Return1Day != nil {
		outcome := *rec.Return1Day * 100 // Convert to percentage
	...
	// Store in knowledge base (this will be used for future accuracy analysis)
...
func (t *RecommendationTracker) fetchPrice(ctx context.Context, symbol string) float64 {
	...
	quotes, err := t.priceService.GetQuotes(ctx, []string{symbol}, model.VNStock)
	if err != nil || len(quotes) == 0 {
		return 0
	}
...
func (t *RecommendationTracker) CleanupOldRecords(ctx context.Context) (int64, error) {
	...
	t.mu.Lock()
	newRecords := make([]model.RecommendationRecord, 0, len(t.records))
...
// Queries from database if available, otherwise uses in-memory cache.
func (t *RecommendationTracker) GetRecommendations(filter model.RecommendationFilter) []model.RecommendationRecord {
	...
	var results []model.RecommendationRecord
...
func (t *RecommendationTracker) getRecommendationsFromDB(ctx context.Context, filter model.RecommendationFilter) ([]model.RecommendationRecord, error) {
	query := `
	...
	var records []model.RecommendationRecord
	for rows.Next() {
		var rec model.RecommendationRecord
		...
		rec.Action = model.RecommendationAction(action)

		// Parse sub_agent_inputs for price and confidence
		var inputs map[string]interface{}
		if err := json.Unmarshal([]byte(subAgentInputsJSON), &inputs); err == nil {
		...
		// Parse recommendation_output for other fields
...
func (t *RecommendationTracker) GetAccuracyByAction(action model.RecommendationAction) model.RecommendationAccuracy {
	t.mu.RLock()
	defer t.mu.RUnlock()

	acc := model.RecommendationAccuracy{Action: action}
	...
	for _, rec := range t.records {
		...
		// For sell signals, a "win" is negative return (price went down)
		isWinFunc := func(ret float64) bool {
			...
			case model.ActionSell:
				return ret < 0
			default:
				return false // hold has no win/loss concept
			}
		}
		...
		if rec.Return7Day != nil {
			...
			// Count totals by confidence bucket
			if rec.ConfidenceScore >= 70 {
				highConfTotal++
			} else if rec.ConfidenceScore >= 40 {
				medConfTotal++
...
func (t *RecommendationTracker) GetSummary() model.RecommendationSummary {
	t.mu.RLock()
	defer t.mu.RUnlock()
	...
	for _, action := range []model.RecommendationAction{model.ActionBuy, model.ActionSell, model.ActionHold} {
	...
	// Compute overall 7-day metrics and find best/worst symbols
	...
	for _, rec := range t.records {
		if rec.Return7Day != nil {
			...
			count7Day++
			if rec.Action == model.ActionBuy && *rec.Return7Day > 0 {
				wins7Day++
			} else if rec.Action == model.ActionSell && *rec.Return7Day < 0 {
				wins7Day++
			}
...
func (t *RecommendationTracker) GetRecordByID(id int64) *model.RecommendationRecord {
	t.mu.RLock()
	defer t.mu.RUnlock()
...
// This is useful for ensuring all cached records are durably stored.
func (t *RecommendationTracker) SaveToDatabase(ctx context.Context) error {
	...
	t.mu.RLock()
	records := make([]model.RecommendationRecord, len(t.records))
	...
	for i := range records {
		...
		if exists {
			// Update existing record with outcomes
			if err := t.updateOutcomeInDB(ctx, rec); err != nil {
...
// This should be called on startup to restore state (Requirement 35.1).
func (t *RecommendationTracker) LoadFromDatabase(ctx context.Context) error {
	if t.db == nil {
		return fmt.Errorf("database not configured")
	}

	// Load recent records (up to cacheMaxSize) from database
	...
	// Merge loaded records with existing cache (avoid duplicates)
	...
	// Update nextID to be greater than max loaded ID
	for _, rec := range t.records {
		if rec.ID >= t.nextID {
			t.nextID = rec.ID + 1
		}
...
// This is useful for batch processing or manual sync (Requirement 35.5).
func (t *RecommendationTracker) FeedbackToKnowledgeBase(ctx context.Context) (int, error) {
	...
	for i := range records {
		...
		// Only feed records with at least 7-day outcomes
		if rec.Return7Day == nil {
			continue
		}
...
// This provides more comprehensive metrics than the in-memory cache.
func (t *RecommendationTracker) GetAccuracyFromDB(ctx context.Context, action model.RecommendationAction) (model.RecommendationAccuracy, error) {
	if t.db == nil {
	...
	// For detailed win/loss analysis, we need to parse the outcome JSON
	// This is done by loading records and computing in memory
	filter := model.RecommendationFilter{
		Action: action,
		Limit:  10000, // Reasonable limit for accuracy computation
	}
...
// This is an alias for CleanupOldRecords for clarity (Requirement 35.6).
