...
func initLLM() {
	// Initialize default required ENV vars checks or general startup logic if needed.
...
func getLLM(ctx context.Context, provider, model, apiKey, awsAccessKey, awsSecretKey, awsRegion string) (llms.Model, error) {
	switch provider {
	case "openai":
		if model == "" {
			model = "gpt-3.5-turbo"
...
// DataAnalyzer creates a synthesis of market data
func analyzeMarketData(ctx context.Context, symbol string) (string, error) {
	if symbol == "" {
		return "No symbol provided for analysis.", nil
	}

	// Fetch real-time data
	quotes, err := vnstockClient.RealTimeQuotes(ctx, []string{symbol})
	if err != nil {
...
// handleChat is the main endpoint orchestrating our "multi-agent" pipeline
func handleChat(c *gin.Context) {
	...
	llm, err := getLLM(ctx, req.Provider, req.Model, req.ApiKey, req.AwsAccessKey, req.AwsSecretKey, req.AwsRegion)
	if err != nil {
	...
	marketContext := ""
	if req.Symbol != "" {
		marketData, err := analyzeMarketData(ctx, req.Symbol)
		if err != nil {
...
// handleModels fetches available models for a given provider
